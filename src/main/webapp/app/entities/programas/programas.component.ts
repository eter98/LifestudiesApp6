...
export class ProgramasComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    if (this.currentSearch) {
      this.programasService
        ...
        .subscribe(
          (res: HttpResponse<IProgramas[]>) => this.paginateProgramas(res.body, res.headers),
  ...
  registerChangeInProgramas() {
    this.eventSubscriber = this.eventManager.subscribe('programasListModification', response => this.reset());
  }
  ...
  protected paginateProgramas(data: IProgramas[], headers: HttpHeaders) {
    this.links = this.parseLinks.parse(headers.get('link'));
    this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
    for (let i = 0; i < data.length; i++) {
      this.programas.push(data[i]);
    }
  }
