...
export class InstitucionComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    if (this.currentSearch) {
      this.institucionService
        ...
        .subscribe(
          (res: HttpResponse<IInstitucion[]>) => this.paginateInstitucions(res.body, res.headers),
  ...
  registerChangeInInstitucions() {
    this.eventSubscriber = this.eventManager.subscribe('institucionListModification', response => this.reset());
  }
  ...
  protected paginateInstitucions(data: IInstitucion[], headers: HttpHeaders) {
    this.links = this.parseLinks.parse(headers.get('link'));
    this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
    for (let i = 0; i < data.length; i++) {
      this.institucions.push(data[i]);
    }
  }
