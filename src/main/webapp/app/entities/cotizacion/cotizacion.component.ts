...
export class CotizacionComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    if (this.currentSearch) {
      this.cotizacionService
        ...
        .subscribe(
          (res: HttpResponse<ICotizacion[]>) => this.paginateCotizacions(res.body, res.headers),
  ...
  registerChangeInCotizacions() {
    this.eventSubscriber = this.eventManager.subscribe('cotizacionListModification', response => this.reset());
  }
  ...
  protected paginateCotizacions(data: ICotizacion[], headers: HttpHeaders) {
    this.links = this.parseLinks.parse(headers.get('link'));
    this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
    for (let i = 0; i < data.length; i++) {
      this.cotizacions.push(data[i]);
    }
  }
