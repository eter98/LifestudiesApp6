...
/**
 * REST controller for managing {@link io.github.jhipster.application.domain.Agencia}.
 */
@RestController
@RequestMapping("/api")
public class AgenciaResource {
    ...
    public ResponseEntity<Agencia> createAgencia(@RequestBody Agencia agencia) throws URISyntaxException {
     ...
     */
    @PutMapping("/agencias")
    public ResponseEntity<Agencia> updateAgencia(@RequestBody Agencia agencia) throws URISyntaxException {
     ...
     */
    @GetMapping("/agencias")
    public ResponseEntity<List<Agencia>> getAllAgencias(AgenciaCriteria criteria, Pageable pageable, @RequestParam MultiValueMap<String, String> queryParams, UriComponentsBuilder uriBuilder) {
    ...
    */
    ...
    /**
     * {@code SEARCH  /_search/agencias?query=:query} : search for the agencia corresponding
     * to the query.
     *
     * @param query the query of the agencia search.
     * @param pageable the pagination information.
     * @param queryParams a {@link MultiValueMap} query parameters.
     * @param uriBuilder a {@link UriComponentsBuilder} URI builder.
     * @return the result of the search.
     */
    @GetMapping("/_search/agencias")
    public ResponseEntity<List<Agencia>> searchAgencias(@RequestParam String query, Pageable pageable, @RequestParam MultiValueMap<String, String> queryParams, UriComponentsBuilder uriBuilder) {
        log.debug("REST request to search for a page of Agencias for query {}", query);
        Page<Agencia> page = agenciaService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(uriBuilder.queryParams(queryParams), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
