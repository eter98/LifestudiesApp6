...
public interface ComentarioBlogRepository extends JpaRepository<ComentarioBlog, Long>, JpaSpecificationExecutor<ComentarioBlog> {
    ...
    @Query("select comentarioBlog from ComentarioBlog comentarioBlog where comentarioBlog.usuario.login = ?#{principal.username}")
    List<ComentarioBlog> findByUsuarioIsCurrentUser();
